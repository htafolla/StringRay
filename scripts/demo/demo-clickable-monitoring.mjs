...
async function demonstrateClickableAgentMonitoring() {
  ...
  console.log(`✅ Architect agent spawned: ${architectAgent.id} (clickable: ${architectAgent.clickable})\n`);
  ...
  console.log(`⏳ Enforcer agent queued: ${enforcerAgent.id} (waiting for dependencies)\n`);
  ...
  console.log(`✅ Librarian agent spawned: ${librarianAgent.id} (clickable: ${librarianAgent.clickable})\n`);
  ...
  const monitorInterval = setInterval(() => {
    ...
    Object.entries(monitoringData).forEach(([id, agent]) => {
      const status = agent.status === 'spawning' ? '🔄 Spawning' :
                    agent.status === 'active' ? '⚡ Active' :
                    agent.status === 'completed' ? '✅ Completed' :
                    agent.status === 'failed' ? '❌ Failed' : '🚫 Cancelled';

      console.log(`  ${id}: ${agent.agentType} - ${status} (${agent.progress}%) ${agent.clickable ? '🖱️ Clickable' : ''}`);
    });
    console.log('');
  }, 2000);
  ...
  Object.entries(monitoringData).forEach(([id, agent]) => {
    const duration = agent.endTime && agent.startTime ? agent.endTime - agent.startTime : 0;
    console.log(`  ${id}: ${agent.agentType}`);
    console.log(`    Status: ${agent.status}`);
    console.log(`    Duration: ${duration}ms`);
    console.log(`    Clickable: ${agent.clickable}`);
    console.log(`    Monitorable: ${agent.monitorable}`);
    if (agent.result) console.log(`    Result: ${typeof agent.result === 'object' ? 'Complex result' : agent.result}`);
  ...
  setTimeout(async () => {
    const cancelled = await enhancedMultiAgentOrchestrator.cancelAgent(cancellableAgent.id);
    console.log(`🚫 Agent ${cancellableAgent.id} cancellation: ${cancelled ? 'SUCCESS' : 'FAILED'}\n`);
  }, 2000);
