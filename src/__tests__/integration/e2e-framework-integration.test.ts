...
import { createSessionCoordinator, SessionCoordinator } from '../../delegation/session-coordinator';
import { createAgentDelegator, AgentDelegator } from '../../delegation/agent-delegator';
...
describe('StrRay Framework End-to-End Integration Tests', () => {
  ...
  describe('Boot → Orchestrator → Session Flow Validation', () => {
    it('should execute complete boot sequence and initialize orchestrator-first architecture', async () => {
      ...
      expect(typeof orchestrator.executeComplexTask).toBe('function');
        ...
        const agentDelegatorInstance = stateManager.get('delegation:agent_delegator');
        const sessionCoordinatorInstance = stateManager.get('delegation:session_coordinator');
        expect(agentDelegatorInstance).toBeDefined();
        expect(sessionCoordinatorInstance).toBeDefined();

        // Verify default session is created
        const defaultSession = stateManager.get('delegation:default_session') as { sessionId: string };
    ...
    it('should validate orchestrator task execution within session context', async () => {
      ...
      // Initialize session
      const sessionResult = sessionCoordinator.initializeSession('test-session-1');
    ...
    it('should handle session lifecycle from boot through orchestrator coordination', async () => {
      ...
      try {
        ...
        const sessionCoordinator = stateManager.get('delegation:session_coordinator') as SessionCoordinator;
    ...
    it('should validate boot failure recovery and orchestrator fallback', async () => {
      ...
      const recoverySession = sessionCoord.initializeSession('recovery-session');
  ...
  describe('Security → Delegation Integration', () => {
    it('should integrate security auditing with agent delegation workflow', async () => {
      ...
      stateManager.set('security:headers_middleware', securityHeadersMiddleware);
      ...
      const sharedContext = sessionCoordinator.getSharedContext(session.sessionId, 'security-audit-results');
    ...
    it('should validate security delegation with processor integration', async () => {
      ...
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('security-processor-session');
      ...
      sessionCoordinator.registerDelegation(session.sessionId, 'security-processor-integration', mockDelegation);
      ...
      const failedProcessors = processorHealth.filter(h => h.status === 'failed');
      expect(failedProcessors).toHaveLength(0);
    });

    it('should handle security delegation failures and recovery', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('security-failure-session');
  ...
  describe('Processor → Session Coordination', () => {
    it('should coordinate processor execution through session management', async () => {
      ...
      sessionCoordinator.shareContext(
        session.sessionId,
        'processor-state',
        { initialized: true, count: processorManager.getProcessorHealth().length },
        'processor-manager'
      );
    ...
    it('should handle processor failures within session coordination', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('processor-failure-session');
      ...
      vi.spyOn(processorManager, 'initializeProcessors').mockResolvedValueOnce(false);
    ...
    it('should validate processor health monitoring through session coordination', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('health-monitor-session');
    ...
    it('should coordinate processor state transitions across session boundaries', async () => {
      ...
      const context1 = sessionCoordinator1.getSharedContext(session1.sessionId, 'processor-transition');
      const context2 = sessionCoordinator2.getSharedContext(session2.sessionId, 'processor-transition');
  ...
  describe('Complete Workflow Testing', () => {
    it('should execute full end-to-end workflow from boot to completion', async () => {
      ...
      try {
        ...
        const sessionCoordinator = stateManager.get('delegation:session_coordinator') as SessionCoordinator;
        const workflowSession = sessionCoordinator.initializeSession('full-workflow-session');

        // Phase 3: Security Integration
        const securityAuditor = new SecurityAuditor();
        const auditResult = await securityAuditor.auditProject(process.cwd());

        const workflowSessionCoordinator = stateManager.get('delegation:session_coordinator') as SessionCoordinator;
        ...
        const processorManager = stateManager.get('processor:manager') as ProcessorManager;
        const processorHealth = processorManager.getProcessorHealth();

        workflowSessionCoordinator.shareContext(
          workflowSession.sessionId,
          'processor-health',
          { healthy: processorHealth.filter(h => h.status === 'healthy').length },
          'processor-manager'
        );
        ...
        const agentDelegator = stateManager.get('delegation:agent_delegator') as AgentDelegator;
...
const finalAnalysis = { strategy: 'single-agent', agents: ['architect'], complexity: { score: 0.5, level: 'moderate', recommendedStrategy: 'single-agent', estimatedAgents: 1, reasoning: ['Test environment'] }, metrics: { fileCount: 1, changeVolume: 10, operationType: 'analyze', dependencies: 0, riskLevel: 'low', estimatedDuration: 5 }, estimatedDuration: 100 };
        ...
        const finalStatus = sessionCoordinator.getSessionStatus(workflowSession.sessionId);
        expect(finalStatus!.active).toBe(true);

        // Cleanup session
        sessionCoordinator.cleanupSession(workflowSession.sessionId);
        const cleanedStatus = sessionCoordinator.getSessionStatus(workflowSession.sessionId);
    ...
    it('should handle complex multi-agent coordination scenarios', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('multi-agent-session');

      // Initialize multiple agents
      const agents = ['architect', 'enforcer', 'security-auditor', 'test-architect'];
    ...
    it('should validate error recovery and system resilience', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('resilience-session');
      ...
      // Mock task failure
      const originalDelegate = orchestrator['delegateToSubagent'].bind(orchestrator);
      orchestrator['delegateToSubagent'] = vi.fn().mockImplementation((task) => {
      ...
      const recoverySession = sessionCoordinator.initializeSession('recovery-session');
      expect(recoverySession.sessionId).toBe('recovery-session');
    ...
    it('should validate performance under concurrent workflow load', async () => {
      ...
      const sessions: Array<{ sessionId: string; createdAt: Date; agentCount: number }> = [];

      for (let i = 0; i < sessionCount; i++) {
        const session = sessionCoordinator.initializeSession(`performance-session-${i}`);
  ...
  describe('Error Handling and Recovery', () => {
    it('should handle boot sequence failures gracefully', async () => {
      ...
      const sessionCoord = createSessionCoordinator(recoveryStateManager);
      const recoverySession = sessionCoord.initializeSession('recovery-after-boot-failure');
    ...
    it('should recover from session coordination failures', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('failure-recovery-session');
    ...
    it('should handle processor cascade failures', async () => {
      ...
      const registeredProcessors = Array.from(processorManager['processors'].values());
      expect(registeredProcessors.length).toBe(2);
    });

    it('should validate system integrity after error recovery', async () => {
      const sessionCoordinator = createSessionCoordinator(stateManager);
      const session = sessionCoordinator.initializeSession('integrity-check-session');
      ...
      const sharedData = sessionCoordinator.getSharedContext(session.sessionId, 'integrity-test');
  ...
  describe('Performance and Load Testing', () => {
    it('should maintain performance under high concurrent load', async () => {
      ...
      for (let i = 0; i < sessionCount; i++) {
        const session = sessionCoordinator.initializeSession(`memory-test-session-${i}`);
        sessions.push(session);
      ...
      sessions.forEach(session => {
        ...
        if (status) {
          ...
          console.log(`Session ${session.sessionId} not found (expected under load)`);
        }
      });
    ...
    it('should benchmark orchestrator performance across different task complexities', async () => {
      ...
      const testCases = [
        ...
        },
        {
          ...
          tasks: [
            { id: 'medium-1', description: 'Medium task 1', subagentType: 'architect' },
            { id: 'medium-2', description: 'Medium task 2', subagentType: 'enforcer', dependencies: ['medium-1'] },
            { id: 'medium-3', description: 'Medium task 3', subagentType: 'security-auditor', dependencies: ['medium-2'] }
          ]
        },
        {
          name: 'High Complexity',
          tasks: [
            { id: 'high-1', description: 'High task 1', subagentType: 'architect' },
            { id: 'high-2', description: 'High task 2', subagentType: 'enforcer', dependencies: ['high-1'] },
            { id: 'high-3', description: 'High task 3', subagentType: 'security-auditor', dependencies: ['high-2'] },
            { id: 'high-4', description: 'High task 4', subagentType: 'test-architect', dependencies: ['high-3'] },
            { id: 'high-5', description: 'High task 5', subagentType: 'bug-triage-specialist', dependencies: ['high-4'] }
          ]
        }
      ];
      ...
      const simpleResult = benchmarkResults.find(r => r.name === 'Simple Task');
      const mediumResult = benchmarkResults.find(r => r.name === 'Medium Complexity');
      const highResult = benchmarkResults.find(r => r.name === 'High Complexity');

      expect(simpleResult!.duration).toBeLessThanOrEqual(mediumResult!.duration);
      expect(mediumResult!.duration).toBeLessThanOrEqual(highResult!.duration);

      console.log('Benchmark Results:');
      benchmarkResults.forEach(result => {
        console.log(`${result.name}: ${result.taskCount} tasks, ${result.duration}ms total, ${result.avgTaskTime.toFixed(2)}ms avg per task`);
      });
    });
  });
});