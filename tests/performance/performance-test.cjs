...
try {
  ...
  if (fs.existsSync(distPath)) {
    ...
    files.forEach(file => {
      if (file.endsWith('.js')) {
        ...
        try {
          ...
          console.log(`  ${file}: ${(stats.size / 1024).toFixed(2)} KB (${(gzippedSize / 1024).toFixed(2)} KB gzipped)`);
        } catch {
    ...
    const withinLimits = totalSize < 2 * 1024 * 1024 && totalGzipped < 700 * 1024;
    console.log(`  Performance budget: ${withinLimits ? '✅ PASSED' : '❌ FAILED'}\n`);
  } else {
