...
async function runAllSimulations() {
  ...
  try {
    ...
    console.log(
      `   Efficiency: ${(distributedResult.coordinationEfficiency * 100).toFixed(1)}%`,
    );
    console.log(
      `   Network Overhead: ${distributedResult.networkOverhead} calls\n`,
    );
    ...
    console.log(
      `   Persisted: ${persistenceResult.persistedSessions}/${persistenceResult.totalSessions}`,
    );
    console.log(
      `   Recovered: ${persistenceResult.recoveredSessions}/${persistenceResult.corruptedSessions}`,
    );
    console.log(
      `   Data Loss: ${(persistenceResult.dataLoss * 100).toFixed(1)}%`,
    );
    console.log(
      `   Recovery Time: ${persistenceResult.averageRecoveryTime}ms\n`,
    );
    ...
    console.log(
      `   Recovered: ${recoveryResult.recoveredSessions}/${recoveryResult.failedSessions}`,
    );
    console.log(`   Cascade Failures: ${recoveryResult.cascadeFailures}`);
    console.log(
      `   Recovery Efficiency: ${(recoveryResult.recoveryEfficiency * 100).toFixed(1)}%`,
    );
    console.log(
      `   Average Recovery Time: ${recoveryResult.averageRecoveryTime}ms\n`,
    );

    // Summary
    const allSuccessful =
      distributedResult.success &&
      persistenceResult.success &&
      recoveryResult.success;
    console.log("🎯 SIMULATION SUITE SUMMARY:");
    console.log(
      `Overall Success: ${allSuccessful ? "✅ ALL PASSED" : "❌ ISSUES DETECTED"}`,
    );
    console.log("Session Management Validation: COMPLETE ✅");

    const totalIssues =
      distributedResult.issues.length +
      persistenceResult.issues.length +
      recoveryResult.issues.length;
    if (totalIssues > 0) {
      console.log(
