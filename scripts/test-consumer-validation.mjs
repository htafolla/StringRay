...
class ConsumerValidator {
    ...
    async validateAll() {
        ...
        console.log('=====================================');
        console.log(`Environment: ${this.isConsumerEnvironment ? 'Consumer' : 'Development'}`);
        console.log('');
        ...
        const tests = [
            ...
            },
            {
                ...
                command: 'node node_modules/strray-ai/dist/cli/index.js auth login --help',
            ...
            },
            {
                ...
                description: 'Tests that the orchestrator can initialize and process tasks',
                timeout: 5000
            },
            {
                ...
                description: 'Tests session manager initialization and basic operations',
                timeout: 3000
            },
            {
            ...
            },
            {
                ...
                description: 'Tests rules engine initialization and basic rule processing',
                timeout: 3000
            },
            {
                name: 'Session Cache',
            ...
            },
            {
                ...
                description: 'Tests that specialized AI agents are loaded and available',
                timeout: 5000
            },
            {
    ...
    async validateConsumerReadiness() {
        ...
        const requiredFiles = [
            ...
            { path: 'node_modules/strray-ai/dist/plugin/plugins/stringray-codex-injection.js', description: 'Main plugin file' },
            { path: 'node_modules/strray-ai/dist/cli/index.js', description: 'CLI entry point' }
        ];
        ...
        try {
            const config = JSON.parse(fs.readFileSync('.opencode/oh-my-opencode.json', 'utf8'));
            const pluginArray = config.plugins || config.plugin || [];
            const hasStringRayPlugin = Array.isArray(pluginArray) &&
                pluginArray.some(plugin => typeof plugin === 'string' && plugin.includes('strray'));
        ...
        try {
            const config = JSON.parse(fs.readFileSync('.opencode/oh-my-opencode.json', 'utf8'));
            const sisyphusDisabled = config.sisyphus_agent?.disabled === true ||
    ...
    async validateFrameworkComponents() {
        ...
        const components = [
            { path: '../dist/plugin/orchestrator.js', export: 'StringRayOrchestrator' },
            { path: '../dist/plugin/state/state-manager.js', export: 'StringRayStateManager' },
            { path: '../dist/plugin/plugins/stringray-codex-injection.js', export: 'default' }
        ];
    ...
    async validateMCPConfiguration() {
        ...
        try {
            ...
            for (const [name, config] of Object.entries(mcpConfig.mcpServers)) {
                if (config.args && config.args.some(arg => arg.includes('node_modules/strray-ai/dist/mcps/'))) {
                    validPaths++;
                }
            }
    ...
    async validatePostinstallEndToEnd() {
        ...
        const checks = [
            ...
            () => fs.existsSync('node_modules/strray-ai/dist/plugin/plugins/stringray-codex-injection.js'),
            () => fs.existsSync('node_modules/strray-ai/dist/cli/index.js'),

            // JSON validity checks
            () => {
                try { JSON.parse(fs.readFileSync('.mcp.json', 'utf8')); return true; }
                catch { return false; }
            },
            () => {
                try { JSON.parse(fs.readFileSync('opencode.json', 'utf8')); return true; }
                catch { return false; }
            },
            () => {
                try { JSON.parse(fs.readFileSync('.opencode/oh-my-opencode.json', 'utf8')); return true; }
                catch { return false; }
            },
            ...
            () => {
                try {
                    const config = JSON.parse(fs.readFileSync('.opencode/oh-my-opencode.json', 'utf8'));
                    return config.sisyphus_agent?.disabled === true ||
                           (config.disabled_agents && config.disabled_agents.includes('Sisyphus'));
                } catch { return false; }
            },
                ...
                try {
                    ...
                    return Object.values(servers).every(server =>
                        server.args && server.args.some(arg =>
                            typeof arg === 'string' && arg.includes('node_modules/strray-ai/dist/')
                        )
                    );
                } catch { return false; }
            }
    ...
    async validateOrchestratorFunctionality() {
        ...
        try {
            const { StringRayOrchestrator } = await import('../dist/plugin/orchestrator.js');
            const orchestrator = new StringRayOrchestrator({ maxConcurrentTasks: 1 });

            // Check that orchestrator has required methods
            const hasExecuteMethod = typeof orchestrator.executeComplexTask === 'function';
            const hasConfig = orchestrator.config && typeof orchestrator.config === 'object';
    ...
    async validateSessionManagement() {
        ...
        try {
            const { StringRayStateManager } = await import('../dist/plugin/state/state-manager.js');
    ...
    async validateComplexOrchestration() {
        ...
        try {
            ...
            const hasValidStructure = testTasks.every(task =>
                task.id && task.description && task.subagentType
            );
    ...
    async validatePromptProcessing() {
        ...
        try {
            ...
            const canHandlePrompts = typeof orchestrator.executeComplexTask === 'function';
            ...
            const isValidPrompt = typeof testPrompt === 'string' && testPrompt.length > 0;
    ...
    async validateRulesEngine() {
        ...
        try {
            ...
            const { frameworkLogger } = await import('../dist/plugin/framework-logger.js');
            ...
            const hasValidRuleStructure = testRule.condition && testRule.action && testRule.priority;
    ...
    async validateSessionCache() {
        ...
        try {
            ...
            const canInitializeCache = sessionManager && typeof sessionManager === 'object';
    ...
    async validateProcessorPipeline() {
        ...
        try {
            ...
            const hasPipelineStructure = testPipeline.preProcessors &&
    ...
    async validateFrameworkLogger() {
        ...
        try {
            ...
            const hasInstance = frameworkLogger && typeof frameworkLogger === 'object';
            ...
            const hasValidLogStructure = testLogEntry.component &&
                                       testLogEntry.action &&
                                       testLogEntry.status;
    ...
    async validateSecurityComponents() {
        ...
        try {
            ...
            const hasSecurityStructure = securityConfig.inputValidation &&
                                       securityConfig.outputSanitization;
    ...
    async validatePerformanceMonitoring() {
        ...
        try {
            ...
            const hasPerformanceStructure = performanceConfig.metricsCollection &&
    ...
    async validateAgentLoading() {
        ...
        try {
            ...
            const hasAgentConfig = ohMyOpencodeConfig.agents || ohMyOpencodeConfig.agent;
            const hasSomeAgents = hasAgentConfig || ohMyOpencodeConfig.disabled_agents;
            ...
            const hasAgentServers = agentRelatedServers.some(server =>
    ...
    printSummary() {
        ...
        if (this.results.failed.length === 0) {
            ...
            console.log('⚠️  Some consumer tests failed. Please fix before publishing.');
