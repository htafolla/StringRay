...
describe('EscalationEngine', () => {
  ...
  describe('evaluateEscalation', () => {
    ...
    it('should escalate to manual-intervention at threshold', async () => {
      ...
      expect(result?.reason).toContain('Manual intervention threshold exceeded');
    });
  ...
  describe('Incident Management', () => {
    ...
    it('should resolve incidents', () => {
      ...
      const resolved = engine.resolveIncident(incidentId, 'Issue fixed manually');
    ...
    it('should return active incidents only', async () => {
      ...
      await engine.evaluateEscalation({ ...mockContext, commitSha: 'def456' }, 3, 'Error 2', []);

      const activeIncidents = engine.getActiveIncidents();
      expect(activeIncidents.length).toBe(2);

      // Resolve one
      const resolved = engine.resolveIncident(activeIncidents[0].id, 'Resolved');
      expect(resolved).toBe(true);
  ...
  describe('Statistics', () => {
    it('should provide escalation statistics', async () => {
      await engine.evaluateEscalation(mockContext, 2, 'Manual intervention', []);
      await engine.evaluateEscalation({ ...mockContext, commitSha: 'def456' }, 3, 'Rollback', []);
      await engine.evaluateEscalation({ ...mockContext, commitSha: 'ghi789' }, 5, 'Emergency', []);
  ...
  describe('Configuration', () => {
    it('should use custom configuration', async () => {
      ...
      const result = await customEngine.evaluateEscalation(mockContext, 2, 'Test', []);
      expect(result).toBeNull();
    });
  });
});