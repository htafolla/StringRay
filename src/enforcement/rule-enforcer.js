...
export class RuleEnforcer {
     ...
     */
    mapCodexSeverity(codexSeverity) {
     ...
     */
    createCodexValidator(codexTerm) {
    ...
    initializeRules() {
        ...
        this.addRule({
            ...
            description: "Console.log must be used only for debugging in dev mode - retained logs must use framework logger",
            category: "code-quality",
    ...
    async validateOperation(operation, context) {
        ...
        await frameworkLogger.log('rule-enforcer', '-debug-applicablerules-length-applicable-rules-for', 'info', { message: `🔍 DEBUG: ${applicableRules.length} applicable rules for operation '${operation}': ${applicableRules.map((r) => r.id).join(", ")}`,
        });
        const results = [];
        const errors = [];
        const warnings = [];
    ...
    async validateConsoleLogUsage(context) {
        const { newCode } = context;
