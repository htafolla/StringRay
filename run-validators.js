...
async function runValidatorTests() {
  ...
  try {
    ...
    const sessionMonitor = createSessionMonitor(stateManager, sessionCoordinator, null);
    ...
    const migrationPlan = {
      ...
      migrationSteps: ['backup_current_state', 'update_coordinator', 'transfer_dependencies', 'verify_migration'],
      rollbackSteps: ['restore_backup']
    };

    const migrationValidation = await migrationValidator.validateMigrationPlan(migrationPlan);
    ...
    const coordinationValidator = new SessionCoordinationValidator(sessionCoordinator, sessionMonitor);

    const coordinationValidation = await coordinationValidator.validateCommunicationPatterns(sessionId);
    console.log('✅ Communication Pattern Validation:');
    console.log(`   Valid: ${coordinationValidation.valid}`);
    console.log(`   Total Messages: ${coordinationValidation.metrics.totalMessages}`);
    console.log(`   Coordination Efficiency: ${(coordinationValidation.metrics.coordinationEfficiency * 100).toFixed(1)}%\n`);

    // 3. Test Security Validator
    console.log('🧪 3. Testing SessionSecurityValidator...');
    const securityValidator = new SessionSecurityValidator(sessionCoordinator, stateManager, null);

    const accessValidation = await securityValidator.validateAccessControl(sessionId);
    console.log('✅ Access Control Validation:');
    console.log(`   Valid: ${accessValidation.valid}`);
    console.log(`   Read Access Agents: ${accessValidation.permissions.readAccess.length}`);
    console.log(`   Admin Access Agents: ${accessValidation.permissions.adminAccess.length}\n`);

    // Summary
    const allValid = migrationValidation.valid && coordinationValidation.valid && accessValidation.valid;
    console.log('🎯 VALIDATOR TEST SUMMARY:');
    console.log(`Overall Success: ${allValid ? '✅ ALL PASSED' : '⚠️ SOME ISSUES'}`);
    console.log('Session Management Validation: OPERATIONAL ✅');

    const totalIssues = migrationValidation.errors.length + coordinationValidation.issues.length + accessValidation.issues.length;
    if (totalIssues > 0) {
