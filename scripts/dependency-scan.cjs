  ...
  scan() {
    ...
    try {
      ...
      if (auditResult.metadata && auditResult.metadata.vulnerabilities) {
        ...
        console.log(`   Total Dependencies: ${auditResult.metadata.totalDependencies || 'Unknown'}`);
        ...
        if ((vuln.critical || 0) > 0 || (vuln.high || 0) > 0) {
          console.log('🚨 HIGH PRIORITY: Critical or High severity vulnerabilities found!');
          ...
          auditResult.actions.forEach((action, index) => {
            ...
            if (action.resolves && action.resolves.length > 0) {
              action.resolves.forEach(resolve => {
                console.log(`      - ${resolve.path} (${resolve.dev ? 'dev' : 'prod'})`);
              });
            }
            console.log('');
          });
