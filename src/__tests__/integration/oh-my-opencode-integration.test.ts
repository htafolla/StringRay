  ...
  loadPlugin: async (pluginPath: string) => {
    ...
    const mockPlugin = {
      ...
      'tool.execute.before': async (input: any) => {
        ...
        if (content.includes('while(true)') || content.includes('for(;;)') || content.includes('while (true)')) {
          throw new Error('Codex violation: Infinite loop detected - violates termination principle');
        }
      },
      config: async () => {
...
describe('Oh-My-OpenCode Integration', () => {
  ...
  test('should inject codex into system prompts', async () => {
    ...
    expect(output.system.some((item: string) => item.includes('StrRay Codex'))).toBe(true);
  });
  ...
  test('should block tool execution with TODO comments', async () => {
    ...
    await expect(mockOhMyOpenCode.executeTool('write', {
      filePath: 'test.ts',
  ...
  test('should track tool execution calls', async () => {
    ...
    expect(mockOhMyOpenCode.toolCalls[mockOhMyOpenCode.toolCalls.length - 1].tool).toBe('read');
  });
  ...
  test('should handle multiple tool executions', async () => {
    const executions = [
      ...
      mockOhMyOpenCode.executeTool('write', { filePath: 'file2.ts', content: 'console.log("test");' }),
      mockOhMyOpenCode.executeTool('edit', { filePath: 'file3.ts', oldString: 'old', newString: 'new' })
    ];
