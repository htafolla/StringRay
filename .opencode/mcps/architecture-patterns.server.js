      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description:
              "Evaluate the pros and cons of an architectural decision",
            inputSchema: {
              type: "object",
              properties: {
                decision: {
...
${recommendations.bestPractices.map((practice) => `- ${practice}`).join("\n")}`,
  ...
  async designSystemArchitecture(args = {}) {
    ...
    try {
      const design = this.createSystemArchitecture(
        requirements,
        technologies,
        scale,
      );
...
${evaluation.implementation.map((consideration) => `- ${consideration}`).join("\n")}`,
      ...
      throw new Error(
        `Failed to evaluate architecture decision: ${error.message}`,
      );
    }
  }
        ...
        patterns: [
          {
            ...
            whenToUse:
              "Complex state interactions, large teams, debugging needs",
