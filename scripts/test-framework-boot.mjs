...
class FrameworkBootTest {
    ...
    async testOrchestratorImport() {
        ...
        try {
            ...
            if (orchestrator && typeof orchestrator.executeComplexTask === 'function') {
                ...
                this.results.failed.push({ test: 'Orchestrator Import', error: 'Missing methods' });
            }
        } catch (error) {
            console.log(`  ❌ Orchestrator import failed: ${error.message}`);
            this.results.failed.push({ test: 'Orchestrator Import', error: error.message });
        }
    }
    ...
    async testStateManagerImport() {
        ...
        try {
            ...
            if (stateManager && typeof stateManager.get === 'function') {
                console.log('  ✅ State Manager imported and instantiated successfully');
                this.results.passed.push('State Manager Import');
            } else {
                console.log('  ❌ State Manager missing required methods');
                this.results.failed.push({ test: 'State Manager Import', error: 'Missing methods' });
            }
        } catch (error) {
            console.log(`  ❌ State Manager import failed: ${error.message}`);
            this.results.failed.push({ test: 'State Manager Import', error: error.message });
        }
    }
    ...
    async testProcessorManagerImport() {
        ...
        try {
            const { ProcessorManager } = await import('../dist/plugin/processors/processor-manager.js');

            if (ProcessorManager && typeof ProcessorManager.prototype.registerProcessor === 'function') {
                ...
                this.results.failed.push({ test: 'Processor Manager Import', error: 'Class not defined' });
            }
        } catch (error) {
            console.log(`  ❌ Processor Manager import failed: ${error.message}`);
            this.results.failed.push({ test: 'Processor Manager Import', error: error.message });
        }
    }
    ...
    async testFrameworkLoggerImport() {
        ...
        try {
            ...
            if (frameworkLogger && typeof frameworkLogger.log === 'function') {
                ...
                this.results.failed.push({ test: 'Framework Logger Import', error: 'Missing methods' });
            }
        } catch (error) {
            console.log(`  ❌ Framework Logger import failed: ${error.message}`);
            this.results.failed.push({ test: 'Framework Logger Import', error: error.message });
        }
    }
    ...
    async testPluginLoading() {
        ...
        try {
            ...
            const pluginModule = await import('../dist/plugin/plugins/stringray-codex-injection.js');
            ...
            if (pluginModule && typeof pluginModule.default === 'function') {
                ...
                this.results.failed.push({ test: 'Plugin Loading', error: 'Not properly exported' });
            }
        } catch (error) {
            console.log(`  ❌ Plugin loading failed: ${error.message}`);
            this.results.failed.push({ test: 'Plugin Loading', error: error.message });
        }
    }
...
bootTest.testFrameworkBoot().then(success => {
