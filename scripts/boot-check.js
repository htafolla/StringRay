...
class BootChecker {
	...
	async checkEnvironment() {
		...
		if (!nodeVersion.startsWith("v18") && !nodeVersion.startsWith("v20")) {
			throw new Error(`Unsupported Node.js version: ${nodeVersion}. Requires v18+`);
		}
		...
		if (!hasCodex) {
			throw new Error("No codex files found. Expected src/agents_template.md or AGENTS.md");
		}
	...
	async checkLogging() {
		...
		const logFiles = files.filter((f) => f.startsWith("strray-") && f.endsWith(".log"));
	...
	async run() {
		...
		try {
			...
			const logResult = await this.runCheck("Logging System", () => this.checkLogging());
