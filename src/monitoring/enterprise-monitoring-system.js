...
export class EnterpriseMonitoringSystem extends EventEmitter {
    ...
    async start() {
        ...
        await frameworkLogger.log('enterprise-monitoring-system', '-health-check-interval-this-config-healthchecks-in', 'info', { message: `   Health Check Interval: ${this.config.healthChecks.interval}ms`,
        });
        this.isRunning = true;
    ...
    async handleMetricsCollected(data) {
        ...
        if (process.env.DEBUG_MONITORING) {
            await frameworkLogger.log('enterprise-monitoring-system', '-metrics-collected-cpu-data-system-cpu-usage-tofix', 'info', { message: `📊 Metrics collected - CPU: ${data.system.cpu.usage.toFixed(1)}%, Memory: ${data.system.memory.usagePercent.toFixed(1)}%`,
            });
        }
    }
