...
describe("SessionCoordinationValidator", () => {
  ...
  beforeEach(() => {
    ...
    validator = new SessionCoordinationValidator(mockSessionCoordinator, mockSessionMonitor);
  });

  describe("validateCommunicationPatterns", () => {
    it("should validate healthy communication patterns", async () => {
      const result = await validator.validateCommunicationPatterns('healthy-session');
    ...
    it("should detect high conflict rates", async () => {
      const result = await validator.validateCommunicationPatterns('high-conflict-session');

      expect(result.valid).toBe(false);
      expect(result.issues.some(issue => issue.startsWith("High conflict rate:"))).toBe(true);
      expect(result.metrics.conflictRate).toBeGreaterThan(0.1);
    });

    it("should reject inactive sessions", async () => {
      const result = await validator.validateCommunicationPatterns('inactive-session');
    ...
    it("should reject non-existent sessions", async () => {
      const result = await validator.validateCommunicationPatterns('non-existent-session');
  ...
  describe("validateDependencyGraphs", () => {
    ...
    it("should detect complex dependency graphs", async () => {
      const result = await validator.validateDependencyGraphs('complex-session');
  ...
  describe("validateContextSharing", () => {
    ...
    it("should handle sessions without shared context", async () => {
      const result = await validator.validateContextSharing('no-shared-session');
    ...
    it("should handle context sharing validation", async () => {
      const result = await validator.validateContextSharing('inconsistent-session');
