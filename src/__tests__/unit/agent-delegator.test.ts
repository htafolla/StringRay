...
import { AgentDelegator, DelegationRequest, createAgentDelegator } from '../../delegation/agent-delegator';
...
describe('AgentDelegator', () => {
  ...
  beforeEach(() => {
    ...
    stateManager.set('agent:enforcer', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'enforcer completed' })
    });
    stateManager.set('agent:architect', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'architect completed' })
    });
    stateManager.set('agent:code-reviewer', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'code-reviewer completed' })
    });
    stateManager.set('agent:security-auditor', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'security-auditor completed' })
    });
    stateManager.set('agent:test-architect', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'test-architect completed' })
    });
    stateManager.set('agent:refactorer', {
      execute: vi.fn().mockResolvedValue({ success: true, result: 'refactorer completed' })
    });
  });
  ...
  describe('analyzeDelegation', () => {
    ...
    it('should determine conflict resolution strategy', async () => {
      ...
      expect(['majority_vote', 'expert_priority', 'consensus']).toContain(result.conflictResolution);
    });
  ...
  describe('executeDelegation', () => {
    ...
    it('should handle execution errors gracefully', async () => {
      ...
      await expect(agentDelegator.executeDelegation(delegation, request))
    ...
    it('should handle orchestrator-led execution', async () => {
      ...
      const mockOrchestrator = {
        executeComplexTask: vi.fn().mockResolvedValue([{ success: true, result: 'done' }])
      };
      stateManager.set('orchestrator', mockOrchestrator);

      // Mock the agents that will be selected for orchestrator-led execution
      const mockAgent1 = {
        execute: vi.fn().mockResolvedValue({ success: true, result: 'agent1 done' })
      };
      const mockAgent2 = {
        execute: vi.fn().mockResolvedValue({ success: true, result: 'agent2 done' })
      };
      stateManager.set('agent:security-auditor', mockAgent1);
      stateManager.set('agent:enforcer', mockAgent2);
  ...
  describe('private methods via public interface', () => {
    ...
    it('should resolve multi-agent conflicts', async () => {
      ...
      const mockAgent1 = {
        execute: vi.fn().mockResolvedValue({ result: 'option1', consensus: false, confidence: 0.7 })
      };
      const mockAgent2 = {
        execute: vi.fn().mockResolvedValue({ result: 'option2', consensus: true, confidence: 0.9 })
      };
  ...
  describe('comprehensive delegation logic tests', () => {
    ...
    it('should handle delegation with priority levels', async () => {
      ...
      const delegation = await agentDelegator.analyzeDelegation(highPriorityRequest);
  ...
  describe('agent capability matching tests', () => {
    ...
    it('should select highest performance agents for critical tasks', async () => {
      agentDelegator.updateAgentCapability('security-auditor', { performance: 100 });
    ...
    it('should handle agent capability updates dynamically', () => {
      ...
      const enforcerBefore = initialCapabilities.find(a => a.name === 'enforcer');
      ...
      const enforcerAfter = updatedCapabilities.find(a => a.name === 'enforcer');
    ...
    it('should match multiple agents for complex multi-disciplinary tasks', async () => {
      const request: DelegationRequest = {
        ...
        description: 'Complete application development with security and testing',
        context: {
  ...
  describe('performance-based selection validation', () => {
    ...
    it('should balance load across agents with similar capabilities', async () => {
      ...
      const requests = [
        { operation: 'review', description: 'Review 1', context: { files: ['file1.ts'] } },
        { operation: 'design', description: 'Design 1', context: { files: ['file2.ts'] } }
      ];
