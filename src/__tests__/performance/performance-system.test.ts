/**
 * StrRay Framework v1.0.0 - Performance System Tests
 *
...
describe('Performance Budget Enforcer', () => {
  ...
  it('should check budget compliance', async () => {
    ...
    const runtimeMetrics = performanceBudgetEnforcer.measureRuntimePerformance();
    const vitals = await performanceBudgetEnforcer.measureWebVitals();

    const violations = performanceBudgetEnforcer.checkBudgetCompliance(bundleMetrics, vitals, runtimeMetrics);
...
describe('Performance Regression Tester', () => {
  ...
  it('should run regression test', async () => {
    const result = await performanceRegressionTester.runRegressionTest(mockTest);
...
describe('Performance Budget Compliance', () => {
  it('should detect bundle size violations', async () => {
    ...
    const violations = performanceBudgetEnforcer.checkBudgetCompliance(oversizedBundle, vitals, runtime);

    expect(violations.length).toBeGreaterThan(0);
    const bundleViolation = violations.find(v => v.metric.includes('bundle-size'));
  ...
  it('should detect web vitals violations', async () => {
    ...
    const slowVitals = {
      firstContentfulPaint: PERFORMANCE_BUDGET.webVitals.firstContentfulPaint * 1.5, // 150% of budget
    ...
    const violations = performanceBudgetEnforcer.checkBudgetCompliance(bundle, slowVitals, runtime);

    const fcpViolation = violations.find(v => v.metric === 'first-contentful-paint');
    const ttiViolation = violations.find(v => v.metric === 'time-to-interactive');
