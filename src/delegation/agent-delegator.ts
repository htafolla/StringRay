...
import { ComplexityAnalyzer, ComplexityScore, ComplexityMetrics } from './complexity-analyzer';
...
export class AgentDelegator {
   ...
   */
  async analyzeDelegation(request: DelegationRequest): Promise<DelegationResult> {
   ...
   */
  async executeDelegation(delegation: DelegationResult, request: DelegationRequest): Promise<any> {
  ...
  private selectAgents(complexity: ComplexityScore, request: DelegationRequest): string[] {
  ...
  private selectSingleAgent(agents: AgentCapability[], request: DelegationRequest): string[] {
  ...
  private selectMultiAgent(agents: AgentCapability[], count: number, request: DelegationRequest): string[] {
    const operation = (request.operation || '').toLowerCase();
  ...
  private selectOrchestratorLed(agents: AgentCapability[], count: number): string[] {
  ...
  private determineConflictResolution(complexity: ComplexityScore): 'majority_vote' | 'expert_priority' | 'consensus' {
  ...
  private isAgentAvailable(agentName: string): boolean {
    ...
    const activeTasks = this.stateManager.get(`agent:${agentName}:active_tasks`) as number || 0;
    return activeTasks < agent.capacity;
  }

  private async executeSingleAgent(agentName: string, request: DelegationRequest): Promise<any> {
    const currentTasks = this.stateManager.get(`agent:${agentName}:active_tasks`) as number || 0;
    ...
    try {
      ...
      const finalTasks = this.stateManager.get(`agent:${agentName}:active_tasks`) as number || 0;
      this.stateManager.set(`agent:${agentName}:active_tasks`, finalTasks - 1);
    }
  }

  private async executeMultiAgent(agentNames: string[], request: DelegationRequest): Promise<any[]> {
  ...
  private async executeOrchestratorLed(agentNames: string[], request: DelegationRequest): Promise<any> {
    const orchestrator = this.stateManager.get('orchestrator');
    if (!orchestrator) {
      throw new Error('Orchestrator not available for orchestrator-led execution');
    }
  ...
  private async callAgent(agentName: string, request: DelegationRequest): Promise<any> {
    ...
    console.warn(`Agent ${agentName} does not have execute method, simulating response`);
    return { agent: agentName, result: `Executed ${request.description}` };
  }

  private resolveMultiAgentConflicts(results: any[], agentNames: string[]): any[] {
  ...
  private updateDelegationMetrics(result: DelegationResult): void {
    ...
    this.delegationMetrics.averageComplexity =
      (this.delegationMetrics.averageComplexity * (this.delegationMetrics.totalDelegations - 1) + result.complexity.score) /
      this.delegationMetrics.totalDelegations;
  ...
  private recordSuccessfulDelegation(delegation: DelegationResult, duration: number): void {
    this.delegationMetrics.successfulDelegations++;
    this.delegationMetrics.averageDuration =
      (this.delegationMetrics.averageDuration * (this.delegationMetrics.successfulDelegations - 1) + duration) /
      this.delegationMetrics.successfulDelegations;
  }

  private recordFailedDelegation(delegation: DelegationResult, duration: number, error: any): void {
    this.delegationMetrics.failedDelegations++;
    console.error(`❌ Delegation failed: ${error?.message || error}`);
  }

  private logDelegationDecision(result: DelegationResult, request: DelegationRequest): void {
...
export const createAgentDelegator = (stateManager: StrRayStateManager): AgentDelegator => {
