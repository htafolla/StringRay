...
- **oh-my-opencode**: v2.12.0`;
      ...
      expect(result.context!.interweaves).toContain('Error Prevention Interweave');
...
#### 3. Term Three`;
  ...
  if (true) {
    console.log("test";
      ...
      expect(result.errors.some(e => e.includes('Bracket mismatch'))).toBe(true);
    });
    ...
    it('should detect double semicolons', async () => {
      ...
      expect(result.warnings.some(w => w.includes('Double semicolon'))).toBe(true);
    });
