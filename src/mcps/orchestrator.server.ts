...
class StrRayOrchestratorServer {
                ...
                tasks: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      estimatedComplexity: { type: 'number', minimum: 1, maximum: 100 }
                    },
                    required: ['id', 'description', 'type']
                  }
                },
                sessionId: { type: 'string' },
                executionMode: { type: 'string', enum: ['parallel', 'sequential', 'optimized'], default: 'optimized' },
  ...
  private async handleOrchestrateTask(args: any) {
    ...
    console.log('🎯 MCP: Orchestrating complex task:', { description, taskCount: tasks.length, sessionId, executionMode });
...
**Agent Utilization:**
${Object.entries(orchestrationResult.agentUtilization).map(([agent, count]) =>
  `• ${agent}: ${count} tasks`
).join('\n')}
...
**Status:** ${orchestrationResult.success ? '🟢 SUCCESS' : '🔴 ISSUES DETECTED'}`;
  ...
  private async handleAnalyzeComplexity(args: any) {
    ...
    console.log('🔍 MCP: Analyzing task complexity:', { taskCount: tasks.length });
...
${analysis.taskComplexity.map((task, index) =>
  `• Task ${index + 1}: ${task.complexity}/100 (${task.category})`
).join('\n')}

**Agent Assignments:**
${analysis.agentAssignments.map(assignment =>
...
**Parallel Potential:** ${Math.round(analysis.parallelPotential * 100)}%`
  ...
  private async handleGetOrchestrationStatus(args: any) {
    ...
    console.log('📊 MCP: Getting orchestration status:', { sessionId, detailed });
  ...
  private async handleCancelOrchestration(args: any) {
    ...
    console.log('🛑 MCP: Cancelling orchestration:', { sessionId, taskId, force });
  ...
  private async handleOptimizeOrchestration(args: any) {
    ...
    console.log('⚡ MCP: Optimizing orchestration patterns:', { includeHistory, includeRecommendations });
...
${optimizationResults.predictedImprovements.map(i => `• 📈 ${i.metric}: ${i.improvement}%`).join('\n')}`;
  ...
  private async executeOrchestrationPlan(plan: any, sessionId: string, timeout: number) {
    const results = {
      success: true,
      completedTasks: 0,
      failedTasks: 0,
      ...
      for (let i = 0; i < plan.phases.length; i++) {
        ...
        for (const task of phase) {
          try {
            ...
            const agent = this.selectAgentForTask(task);
            results.agentUtilization[agent] = (results.agentUtilization[agent] || 0) + 1;
  ...
  private async analyzeTaskComplexity(tasks: any[]) {
    ...
    analysis.estimatedDuration = this.estimateExecutionDuration({ agentAssignments: agentCounts });
    analysis.parallelPotential = this.calculateParallelPotential(tasks);

    return analysis;
  }

  private calculateTaskComplexity(task: any): { score: number; category: string } {
    ...
    score *= riskMultiplier[(task.riskLevel as keyof typeof riskMultiplier) || 'medium'];
  ...
  private estimateExecutionDuration(plan: any): number {
    ...
    for (const [agent, tasks] of Object.entries(plan.agentAssignments || {})) {
      ...
      const taskCount = Array.isArray(tasks) ? tasks.length : (typeof tasks === 'number' ? tasks : 0);
  ...
  private generateComplexityRecommendations(analysis: any): string[] {
    ...
    const overloadedAgents = analysis.agentAssignments.filter((a: any) => a.utilization > 80);
    if (overloadedAgents.length > 0) {
      recommendations.push(`Consider load balancing for overloaded agents: ${overloadedAgents.map((a: any) => a.agent).join(', ')}`);
    }
  ...
  private calculateAgentUtilization(): Record<string, number> {
    ...
    for (const session of this.activeTasks.values()) {
      for (const [agent, tasks] of Object.entries(session.plan?.agentAssignments || {})) {
        utilization[agent] = (utilization[agent] || 0) + (Array.isArray(tasks) ? tasks.length : (typeof tasks === 'number' ? tasks : 0));
      }
    }
  ...
  private async cancelSession(sessionId: string, force: boolean): Promise<boolean> {
    const session = this.activeTasks.get(sessionId);
  ...
  private async cancelSpecificTask(sessionId: string, taskId: string, force: boolean): Promise<boolean> {
    const session = this.activeTasks.get(sessionId);
  ...
  private async analyzeOrchestrationPatterns(includeHistory: boolean) {
    const data = includeHistory ? this.taskHistory : [this.getOverallOrchestrationStatus()];
    ...
    for (const item of data) {
      ...
      for (const [agent, count] of Object.entries(item.result?.agentUtilization || {})) {
        agentTaskCounts[agent] = (agentTaskCounts[agent] || 0) + (typeof count === 'number' ? count : 0);
      }
    }
    ...
    if (results.averageDuration > 60000) { // 1 minute
...
**Agent Utilization:** ${Object.entries(status.agentUtilization || {}).map(([a, c]) => `${a}:${c}`).join(', ')}`;
    } else {
      // Overall status
