...
interface SecurityVulnerability {
  ...
  category: 'injection' | 'authentication' | 'authorization' | 'cryptography' | 'configuration' | 'data-protection' | 'input-validation';
...
class StrRaySecurityAuditServer {
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                vulnerabilityType: {
                  type: "string",
                  enum: ["injection", "authentication", "authorization", "cryptography", "xss", "csrf", "secrets", "configuration"]
                },
                severity: {
                  type: "string",
  ...
  private async checkVulnerability(args: any): Promise<any> {
    ...
    try {
      ...
      const vulnerabilities = this.analyzeFileForVulnerabilities(content, filePath, language)
        .filter(v => v.category === vulnerabilityType || this.mapVulnTypeToCategory(vulnerabilityType).includes(v.category))
  ...
  private analyzeFileForVulnerabilities(content: string, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
  ...
  private checkInjectionVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];

    // SQL Injection
    if (line.includes('query(') || line.includes('execute(') || line.includes('sql(')) {
  ...
  private checkAuthVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];

    // Weak password policies
    if (line.includes('password') && (line.includes('length') || line.includes('min'))) {
    ...
    if (line.includes('jwt') || line.includes('JWT')) {
      if (!line.includes('expires') && !line.includes('exp') && !line.includes('expiresIn')) {
        vulnerabilities.push({
  ...
  private checkCryptoVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
  ...
  private checkConfigurationVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
  ...
  private checkDataProtectionVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];

    // Sensitive data logging
    if (line.includes('console.log') || line.includes('logger.')) {
      if (line.includes('password') || line.includes('token') || line.includes('secret') || line.includes('key')) {
        vulnerabilities.push({
  ...
  private checkInputValidationVulnerabilities(line: string, lineNum: number, filePath: string, language: string): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
    ...
    if (line.includes('req.body') || line.includes('req.query') || line.includes('req.params')) {
      ...
      const hasValidation = nextLines.some(nextLine =>
        nextLine.includes('validate') || nextLine.includes('sanitize') ||
        nextLine.includes('joi') || nextLine.includes('zod') ||
        nextLine.includes('isEmail') || nextLine.includes('isLength')
      );
  ...
  private getNextLines(filePath: string, startLine: number, count: number): string[] {
    try {
      const content = fs.readFileSync(filePath, 'utf-8');
      const lines = content.split('\n');
      const result: string[] = [];
  ...
  private calculateSecuritySummary(vulnerabilities: SecurityVulnerability[], totalFiles: number): SecurityAuditReport['summary'] {
    const counts = {
  ...
  private analyzeCompliance(vulnerabilities: SecurityVulnerability[], frameworks: string[]): SecurityAuditReport['compliance'] {
  ...
  private generateRecommendations(vulnerabilities: SecurityVulnerability[], compliance: SecurityAuditReport['compliance']): string[] {
  ...
  private generateMarkdownReport(report: SecurityAuditReport, includeRemediation: boolean): string {
  ...
  private generateHTMLReport(report: SecurityAuditReport, includeRemediation: boolean): string {
