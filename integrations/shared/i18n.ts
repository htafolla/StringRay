...
export class StrRayI18n implements I18nInstance {
  ...
  te(key: string): boolean {
    ...
    return message !== undefined || (this.locale !== this.fallbackLocale &&
...
export const formatDate = (date: Date, locale: string = strRayI18n.locale, options?: Intl.DateTimeFormatOptions): string => {
  return new Intl.DateTimeFormat(locale, options).format(date);
};

export const formatNumber = (number: number, locale: string = strRayI18n.locale, options?: Intl.NumberFormatOptions): string => {
  return new Intl.NumberFormat(locale, options).format(number);
};

export const formatRelativeTime = (date: Date, locale: string = strRayI18n.locale): string => {
  const now = new Date();
