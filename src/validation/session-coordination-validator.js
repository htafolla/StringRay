...
export class SessionCoordinationValidator {
    sessionCoordinator;
    sessionMonitor;
    constructor(sessionCoordinator, sessionMonitor) {
     ...
     */
    async validateCommunicationPatterns(sessionId) {
        const issues = [];
     ...
     */
    async validateDependencyGraphs(sessionId) {
        const issues = [];
     ...
     */
    async validateContextSharing(sessionId) {
        const issues = [];
    ...
    findIsolatedAgents(sessionId, communications) {
        const agents = this.sessionCoordinator.getSessionAgents(sessionId);
        ...
        return agents.filter((agent) => !activeAgents.has(agent));
    }
    detectCycles(dependencies) {
        const cycles = [];
        const visited = new Set();
        const recursionStack = new Set();
        const dfs = (agent, path) => {
...
//# sourceMappingURL=session-coordination-validator.js.map