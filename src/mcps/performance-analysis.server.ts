...
class StrRayPerformanceAnalysisServer {
        ...
        tools: [
          {
            ...
            inputSchema: {
              ...
              properties: {
                scope: {
                  type: 'string',
                  enum: ['full', 'runtime', 'build', 'resources', 'bottlenecks'],
                  default: 'full',
  ...
  private async handlePerformanceAnalysis(args: any) {
    ...
    console.log('📊 MCP: Performing performance analysis:', { scope, duration, detailed });
    ...
    try {
      ...
      // Generate summary
      analysisResults.summary = this.generatePerformanceSummary(analysisResults);
...
**Status:** ${results.critical ? '❌ CRITICAL BOTTLENECKS' : '✅ NO CRITICAL ISSUES'}`
  ...
  private async analyzeSpecificBottlenecks(operation: string, threshold: number) {
    const results = {
      bottlenecks: [] as string[],
      recommendations: [] as string[],
      impact: 'Low',
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory() && !item.startsWith('.') && item !== 'node_modules') {
            scanDir(fullPath);
          } else if (stat.isFile() && stat.size > 10 * 1024 * 1024) { // 10MB
