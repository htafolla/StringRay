...
export default async function strrayCodexPlugin(input: {
  ...
  return {
    ...
    config: async (_config: Record<string, unknown>) => {
      ...
      // Register MCP servers for StrRay agents
      ...
      for (const serverName of mcpServerNames) {
        ...
        if (fs.existsSync(serverPath)) {
          mcpServers[serverName] = {
            ...
            env: {
              ...process.env,
              STRRAY_FRAMEWORK_VERSION: "1.0.0",
              STRRAY_CODEX_VERSION: "v1.2.20",
            },
          };
          logger.log(`Registered MCP server: ${serverName}`);
        } else {
          logger.log(`MCP server not found: ${serverPath}`);
        }
      }

      // Run framework initialization
      const initScriptPath = path.join(directory, ".opencode", "init.sh");
      logger.log(`Running init script from directory: ${directory}`);
      logger.log(`Init script path: ${initScriptPath}`);
      logger.log(`Init script exists: ${fs.existsSync(initScriptPath)}`);

      if (fs.existsSync(initScriptPath)) {
        try {
          // For init.sh, let stdout display directly to user instead of capturing it
          const initProcess = spawn("bash", [initScriptPath], {
            cwd: directory,
            stdio: ["ignore", "inherit", "pipe"], // stdout inherits, stderr pipes
          });
          ...
          if (stderr) {
            logger.error(`Framework init stderr: ${stderr}`);
          }

          logger.log("Framework initialization completed successfully");
        } catch (error: unknown) {
          logger.error("Framework initialization failed", error);
        }
      } else {
        logger.error(`Init script not found at: ${initScriptPath}`);
      }
