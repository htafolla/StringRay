...
class StrRaySessionManagementServer {
      ...
      get: (key) => {
        ...
        const mockSessions = {
          "session_001": {
            ...
            activeDelegations: new Map([["del_001", { agents: ["enforcer", "architect"] }]]),
            agentInteractions: new Map([["enforcer", [{ agentName: "enforcer", timestamp: Date.now(), action: "codex-validation", result: "passed", duration: 150, success: true }]]]),
            conflictHistory: [],
            coordinationState: {
              activeAgents: new Set(["enforcer", "architect", "orchestrator"]),
              pendingCommunications: [],
              sharedContext: new Map([["context_001", { type: "codex", version: "1.2.20" }]]),
              sessionMetrics: { totalInteractions: 5, successfulInteractions: 4, failedInteractions: 1, averageResponseTime: 120, conflictResolutionRate: 1.0, coordinationEfficiency: 0.8 }
            },
            isActive: true
          },
          "session_002": {
            ...
            agentInteractions: new Map([["test-architect", [{ agentName: "test-architect", timestamp: Date.now(), action: "test-generation", result: "10 tests created", duration: 200, success: true }]]]),
            conflictHistory: [{ conflictId: "conf_001", timestamp: Date.now(), agents: ["code-reviewer", "security-auditor"], resolution: "consensus", outcome: "approved" }],
            coordinationState: {
              activeAgents: new Set(["test-architect", "code-reviewer"]),
              pendingCommunications: [{ id: "comm_001", fromAgent: "test-architect", toAgent: "code-reviewer", message: "Test coverage analysis complete", timestamp: Date.now(), priority: "medium" }],
              sharedContext: new Map(),
              sessionMetrics: { totalInteractions: 3, successfulInteractions: 3, failedInteractions: 0, averageResponseTime: 80, conflictResolutionRate: 1.0, coordinationEfficiency: 0.9 }
            },
            isActive: true
          }
        };
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          {
            name: "session_list",
            description: "List all OpenCode sessions with optional filtering. Returns session IDs with metadata including message count, date range, and agents used.",
            inputSchema: {
              type: "object",
              properties: {
          ...
          {
            name: "session_read",
            description: "Read messages and history from an OpenCode session. Returns formatted view of session messages with role, timestamp, and content.",
            inputSchema: {
              type: "object",
              properties: {
          ...
          {
            name: "session_search",
            description: "Search for content within OpenCode session messages. Performs full-text search across session messages and returns matching excerpts with context.",
            inputSchema: {
              type: "object",
              properties: {
          ...
          {
            name: "session_info",
            description: "Get metadata and statistics about an OpenCode session. Returns detailed information about a session including message count, date range, agents used, and available data sources.",
            inputSchema: {
              type: "object",
              properties: {
  ...
  async sessionList(args) {
    ...
    console.log(`📋 Session Management: Listing sessions with filters`, { limit, from_date, to_date, project_path });
    ...
    if (from_date) {
      ...
      filteredSessions = filteredSessions.filter(s => s.startTime >= fromTime);
    }
    ...
    const sessionSummaries = filteredSessions.map(session => ({
      sessionId: session.sessionId,
      messages: session.agentInteractions ? Array.from(session.agentInteractions.values()).flat().length : 0,
      first: new Date(session.startTime).toISOString().split('T')[0],
      last: new Date(Math.max(...Array.from(session.agentInteractions?.values() || []).flat().map(i => i.timestamp) || [session.startTime])).toISOString().split('T')[0],
  ...
  async sessionRead(args) {
    ...
    console.log(`📖 Session Management: Reading session ${session_id}`, { include_todos, include_transcript, limit });
    ...
    for (const [agentName, interactions] of session.agentInteractions) {
      for (const interaction of interactions.slice(0, limit || interactions.length)) {
        messages.push({
  ...
  async sessionSearch(args) {
    ...
    console.log(`🔍 Session Management: Searching for "${query}"`, { session_id, case_sensitive, limit });

    const sessions = session_id
      ? [this.stateManager.get(`session:${session_id}`)].filter(Boolean)
      : [this.stateManager.get("session:session_001"), this.stateManager.get("session:session_002")].filter(Boolean);
    ...
    const output = `Found ${results.length} matches${session_id ? ` in session ${session_id}` : ' across sessions'}:\n\n${
      results.map(r => `[${r.sessionId}] Message ${r.messageId} (${r.role})\n...${r.excerpt}...\n`).join('\n')
    }`;
  ...
  async sessionInfo(args) {
    ...
    console.log(`📊 Session Management: Getting info for session ${session_id}`);
    ...
    const messageCount = Array.from(session.agentInteractions.values()).flat().length;
    const firstMessage = Math.min(...Array.from(session.agentInteractions.values()).flat().map(i => i.timestamp));
    const lastMessage = Math.max(...Array.from(session.agentInteractions.values()).flat().map(i => i.timestamp));
