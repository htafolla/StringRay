...
describe('Orchestrator Dependency Handling', () => {
  ...
  it('should execute tasks with dependencies in correct order', async () => {
    const tasks: TaskDefinition[] = [
      ...
      { id: 'task-2', description: 'Process', subagentType: 'librarian', dependencies: ['task-1'] },
      { id: 'task-3', description: 'Validate', subagentType: 'enforcer', dependencies: ['task-2'] }
    ];

    const results = await orchestrator.executeComplexTask('Dependency test', tasks);
  ...
  it('should detect circular dependencies', async () => {
    const circularTasks: TaskDefinition[] = [
      { id: 'task-a', description: 'Task A', subagentType: 'architect', dependencies: ['task-c'] },
      { id: 'task-b', description: 'Task B', subagentType: 'librarian', dependencies: ['task-a'] },
      { id: 'task-c', description: 'Task C', subagentType: 'enforcer', dependencies: ['task-b'] }
    ];

    await expect(orchestrator.executeComplexTask('Circular dependency test', circularTasks))
