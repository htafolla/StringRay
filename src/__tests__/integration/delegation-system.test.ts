...
describe('StrRay Delegation System Integration', () => {
  ...
  beforeEach(() => {
    ...
    const mockAgent = {
      ...
      getCapabilities: vi.fn().mockReturnValue({ name: 'test-architect', performance: 0.8 })
    };
  ...
  describe('Agent Coordination', () => {
    it('should send and receive messages between agents', async () => {
      ...
      await sessionCoordinator.sendMessage(session.sessionId, 'architect', 'enforcer', {
        type: 'design_review',
        data: 'Please review the architectural changes'
      });

      const messages = sessionCoordinator.receiveMessages(session.sessionId, 'enforcer');
    ...
    it('should share and retrieve context data', async () => {
      ...
      sessionCoordinator.shareContext(session.sessionId, 'design_decisions', {
        pattern: 'observer',
        reasoning: 'For loose coupling between auth components'
      }, 'architect');

      const sharedData = sessionCoordinator.getSharedContext(session.sessionId, 'design_decisions');
  ...
  describe('Conflict Resolution', () => {
    it('should resolve conflicts using majority vote', async () => {
      ...
      sessionCoordinator.shareContext(session.sessionId, 'recommendation', 'Option A', 'architect');
      sessionCoordinator.shareContext(session.sessionId, 'recommendation', 'Option A', 'enforcer');
      sessionCoordinator.shareContext(session.sessionId, 'recommendation', 'Option B', 'refactorer');

      const resolved = sessionCoordinator.resolveConflict(session.sessionId, 'recommendation', 'majority_vote');
    ...
    it('should resolve conflicts using expert priority', async () => {
      ...
      sessionCoordinator.shareContext(session.sessionId, 'security_review', 'Approved', 'security-auditor');
      sessionCoordinator.shareContext(session.sessionId, 'security_review', 'Rejected', 'architect');

      const resolved = sessionCoordinator.resolveConflict(session.sessionId, 'security_review', 'expert_priority');
