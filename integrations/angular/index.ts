...
export class StrRayService {
  ...
  private integrationSubject = new BehaviorSubject<StrRayIntegration | null>(
    null,
  );
...
@Component({
  ...
  template: `
    <div class="strray-response" *ngIf="result || loading || error">
      ...
      <div *ngIf="error" class="strray-error">Error: {{ error.message }}</div>
...
export class StrRayAgentResponseComponent {
  ...
  private async executeTask() {
    ...
    this.error = null;
    try {
      this.result = await this.strRayService.executeAgent(
        this.agentName,
        this.task,
      );
...
@NgModule({
  declarations: [StrRayAgentResponseComponent, StrRayValidateDirective],
  exports: [StrRayAgentResponseComponent, StrRayValidateDirective],
  providers: [StrRayService],
})
