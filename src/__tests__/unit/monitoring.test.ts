...
describe('Advanced Monitoring System', () => {
  ...
  test('should aggregate system-wide metrics', () => {
    ...
    expect(systemMetrics.activeAgents).toBeLessThanOrEqual(systemMetrics.totalAgents);
    expect(systemMetrics.completedTasks + systemMetrics.failedTasks).toBe(systemMetrics.totalTasks);
  });
