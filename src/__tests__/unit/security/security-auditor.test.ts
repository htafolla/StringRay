...
describe('SecurityAuditor', () => {
  ...
  describe('auditProject', () => {
    ...
    it('should detect eval() usage as critical vulnerability', async () => {
      ...
      const evalIssue = result.issues.find(issue => issue.category === 'code-injection');
    ...
    it('should detect hardcoded secrets', async () => {
      ...
      mockFs.readFileSync.mockReturnValue('const apiKey = "sk-1234567890abcdef";');

      const result = await auditor.auditProject('/test');

      const secretIssue = result.issues.find(issue => issue.category === 'hardcoded-secrets');
    ...
    it('should detect dangerous imports', async () => {
      ...
      const importIssue = result.issues.find(issue => issue.category === 'dangerous-imports');
    ...
    it('should handle package.json auditing', async () => {
      ...
      expect(result.issues.some(issue => issue.category === 'dependency-management')).toBe(true);
      expect(result.issues.some(issue => issue.category === 'security-practices')).toBe(true);
    });
  ...
  describe('Error Handling', () => {
    it('should handle file read errors gracefully', async () => {
      ...
      expect(result.issues.some(issue => issue.category === 'file-access')).toBe(true);
    });
