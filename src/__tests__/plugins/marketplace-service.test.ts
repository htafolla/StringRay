...
function generateMockAuthor(overrides: Partial<PluginAuthor> = {}): PluginAuthor {
  return {
...
function generateMockSecurity(overrides: Partial<PluginSecurityInfo> = {}): PluginSecurityInfo {
  return {
    verified: faker.datatype.boolean(),
...
function generateMockMetadata(overrides: Partial<PluginMetadata> = {}): PluginMetadata {
  return {
    keywords: faker.helpers.arrayElements(['typescript', 'react', 'node', 'security', 'performance'], 3),
    supportedFrameworks: ['strray'],
    supportedPlatforms: [faker.helpers.arrayElement(['linux', 'macos', 'windows'])],
    supportedArchitectures: [faker.helpers.arrayElement(['x64', 'arm64'])],
    languages: [faker.helpers.arrayElement(['TypeScript', 'JavaScript', 'Python'])],
    runtime: {
...
function generateMockCompatibility(overrides: Partial<PluginCompatibility> = {}): PluginCompatibility {
  return {
...
function generateMockVersion(overrides: Partial<PluginVersion> = {}): PluginVersion {
  return {
...
function generateMockPlugin(overrides: Partial<MarketplacePlugin> = {}): MarketplacePlugin {
  ...
  return {
    ...
    category: faker.helpers.arrayElement(['analytics', 'security', 'performance', 'ui', 'api']),
    tags: faker.helpers.arrayElements(['typescript', 'react', 'node', 'security', 'performance'], 3),
...
describe('Plugin Marketplace Service - Core Functionality', () => {
  ...
  describe('Author-based Plugin Queries', () => {
    it('should retrieve plugins by author ID', async () => {
      ...
      const otherPlugins = Array.from({ length: 2 }, () => generateMockPlugin());
    ...
    it('should filter out unpublished plugins from author results', async () => {
      const author = generateMockAuthor();
      const publishedPlugin = generateMockPlugin({ author, status: 'published' });
  ...
  describe('Category-based Plugin Queries', () => {
    it('should retrieve plugins by category with rating-based sorting', async () => {
      ...
      const categoryPlugins = [
        generateMockPlugin({ category, stats: generateMockStats({ rating: 3.5 }) }),
        generateMockPlugin({ category, stats: generateMockStats({ rating: 4.8 }) }),
        generateMockPlugin({ category, stats: generateMockStats({ rating: 2.1 }) }),
      ];
      const otherPlugins = Array.from({ length: 2 }, () =>
        generateMockPlugin({ category: 'analytics' })
      );
      ...
      expect(result[0].stats.rating).toBeGreaterThanOrEqual(result[1].stats.rating);
    });

    it('should return empty array for categories with no plugins', async () => {
      const result = await service.getPluginsByCategory('non-existent-category');
      expect(result).toEqual([]);
    });
  });
  ...
  describe('Personalized Recommendations', () => {
    it('should provide recommendations based on categories and ratings', async () => {
      const plugins = [
        generateMockPlugin({ category: 'security', stats: generateMockStats({ rating: 4.5 }) }),
        generateMockPlugin({ category: 'analytics', stats: generateMockStats({ rating: 4.2 }) }),
        generateMockPlugin({ category: 'security', stats: generateMockStats({ rating: 3.8 }) }),
        generateMockPlugin({ category: 'ui', stats: generateMockStats({ rating: 4.7 }) }),
        generateMockPlugin({ category: 'security', stats: generateMockStats({ rating: 2.0 }) }),
      ];
    ...
    it('should balance recommendations across categories', async () => {
      const categories = ['security', 'analytics', 'performance'];
      const plugins = categories.flatMap(category =>
        Array.from({ length: 5 }, () =>
          generateMockPlugin({ category, stats: generateMockStats({ rating: 4.0 }) })
        )
      );
...
describe('Plugin Marketplace Service - Search Functionality', () => {
  ...
  describe('Basic Text Search', () => {
    ...
    it('should find plugins by description keywords', async () => {
      ...
      expect(result.plugins.some(p => p.name === 'Analytics Dashboard')).toBe(true);
    });
    ...
    it('should find plugins by tag matches', async () => {
      ...
      expect(result.plugins.some(p => p.tags.includes('typescript'))).toBe(true);
    });
    ...
    it('should perform case-insensitive search', async () => {
      ...
      expect(result.plugins.some(p => p.name === 'Performance Monitor')).toBe(true);
    });
  ...
  describe('Advanced Search Filters', () => {
    it('should filter by category', async () => {
      ...
      expect(result.plugins.some(p => p.name === 'Security Scanner')).toBe(true);
    ...
    it('should filter by tags', async () => {
      ...
      expect(result.plugins.every(p =>
        p.tags.includes('react') || p.tags.includes('ui')
      )).toBe(true);
    });
  ...
  describe('Search Result Sorting', () => {
    ...
    it('should sort by download count', async () => {
      ...
      for (let i = 1; i < result.plugins.length; i++) {
        expect(result.plugins[i-1].stats.downloads).toBeGreaterThanOrEqual(result.plugins[i].stats.downloads);
      }
    });
    ...
    it('should sort by rating', async () => {
      ...
      for (let i = 1; i < result.plugins.length; i++) {
        expect(result.plugins[i-1].stats.rating).toBeGreaterThanOrEqual(result.plugins[i].stats.rating);
      }
    });
    ...
    it('should support ascending sort order', async () => {
      ...
      for (let i = 1; i < result.plugins.length; i++) {
        expect(result.plugins[i-1].stats.rating).toBeLessThanOrEqual(result.plugins[i].stats.rating);
      }
    });
  });
  ...
  describe('Search Facets Generation', () => {
    it('should generate comprehensive search facets', async () => {
      ...
      expect(result.facets).toHaveProperty('licenses');
      ...
      Object.values(result.facets.categories).forEach(count => {
    ...
    it('should accurately count facet occurrences', async () => {
      ...
      const plugins = [
        generateMockPlugin({ category: 'security', license: 'MIT', tags: ['typescript'] }),
        generateMockPlugin({ category: 'security', license: 'MIT', tags: ['typescript'] }),
        generateMockPlugin({ category: 'analytics', license: 'Apache-2.0', tags: ['python'] }),
      ];
      ...
      expect(result.facets.categories.security).toBe(2);
      expect(result.facets.categories.analytics).toBe(1);
      expect(result.facets.licenses.MIT).toBe(2);
      expect(result.facets.licenses['Apache-2.0']).toBe(1);
      expect(result.facets.tags.typescript).toBe(2);
...
describe('Plugin Marketplace Service - Download & Security', () => {
  ...
  describe('Plugin Download Process', () => {
    ...
    it('should update download statistics', async () => {
      ...
      await service.downloadPlugin(plugin.id, plugin.version);

      const updatedPlugin = service['plugins'].get(plugin.id);
      expect(updatedPlugin?.stats.downloads).toBe(11);
      expect(updatedPlugin?.stats.lastDownload).toBeGreaterThan(plugin.stats.lastDownload);
    });
    ...
    it('should generate secure download tokens', async () => {
      ...
      const result1 = await service.downloadPlugin(plugin.id, plugin.version);
      const result2 = await service.downloadPlugin(plugin.id, plugin.version);
  ...
  describe('Download Token Management', () => {
    ...
    it('should handle token expiration correctly', async () => {
      ...
      const result = await service.downloadPlugin(plugin.id, plugin.version);
  ...
  describe('Plugin Reporting System', () => {
    ...
    it('should reject reports for non-existent plugins', async () => {
      ...
      await expect(service.reportPlugin('non-existent', report)).rejects.toThrow('Plugin not found');
    });
    ...
    it('should handle various report types', async () => {
      ...
      const reportTypes: Array<PluginReport['type']> = [
        'security', 'inappropriate', 'malware', 'spam', 'broken', 'other'
      ];
...
describe('Plugin Marketplace Service - Error Handling & Edge Cases', () => {
  ...
  describe('Input Validation Errors (Codex Term 11: Type Safety First)', () => {
    it('should handle null and undefined inputs gracefully', async () => {
      ...
      for (const input of invalidInputs) {
        await expect(service.search(input as any)).rejects.toThrow();
        await expect(service.getPlugin(input as any)).rejects.toThrow();
  ...
  describe('Concurrent Operations', () => {
    it('should handle concurrent plugin registrations', async () => {
      ...
      await Promise.all(plugins.map(plugin => Promise.resolve(service.registerPlugin(plugin))));
    ...
    it('should handle concurrent downloads safely', async () => {
      ...
      const downloadPromises = Array.from({ length: 5 }, () =>
        service.downloadPlugin(plugin.id, plugin.version)
      );
  ...
  describe('Resource Exhaustion Scenarios', () => {
    it('should handle large numbers of plugins efficiently', async () => {
      const largePluginSet = Array.from({ length: 1000 }, () => generateMockPlugin());
    ...
    it('should handle memory pressure from large search results', async () => {
      const plugins = Array.from({ length: 500 }, () => generateMockPlugin({
  ...
  describe('Performance Degradation Scenarios', () => {
    it('should maintain performance with fragmented search index', async () => {
      ...
      const plugins = Array.from({ length: 100 }, () => generateMockPlugin({
...
describe('Plugin Marketplace Service - Integration Scenarios', () => {
  ...
  describe('Complete Plugin Lifecycle', () => {
    it('should support full plugin lifecycle from registration to download', async () => {
      ...
      // 4. Download plugin
      const downloadResult = await service.downloadPlugin(plugin.id, plugin.version);
      expect(downloadResult.success).toBe(true);

      // 5. Verify download stats updated
      const updatedPlugin = service['plugins'].get(plugin.id);
      expect(updatedPlugin?.stats.downloads).toBe(plugin.stats.downloads + 1);
    });
  ...
  describe('Complex Search Scenarios', () => {
    ...
    it('should handle complex multi-criteria searches', async () => {
      ...
      expect(result.plugins[0].stats.downloads).toBeGreaterThanOrEqual(result.plugins[1].stats.downloads);
    });
  ...
  describe('Load Testing Scenarios', () => {
    ...
    it('should maintain data consistency under concurrent modifications', async () => {
      const plugin = generateMockPlugin({ stats: generateMockStats({ downloads: 0 }) });
      service.registerPlugin(plugin);
