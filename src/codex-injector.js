        ...
        hooks: {
            "agent.start": async (sessionId) => {
                ...
                try {
                    ...
                    const stats = getCodexStats(sessionId);
                    if (stats.loaded) {
                        await frameworkLogger.log('codex-injector', '-stringray-codex-loaded-stats-totalterms-terms-sta', 'success', { message: `✅ StringRay Codex loaded: ${stats.totalTerms} terms, ${stats.fileCount} sources`,
                        });
                        await frameworkLogger.log("codex-injector", "codex context loaded successfully", "success", { jobId, ...stats });
                    }
                    else {
                        await frameworkLogger.log('codex-injector', '-no-codex-files-found-checked-strray-codex-json-co', 'info', { message: "⚠️  No codex files found. Checked: .strray/codex.json, codex.json, src/codex.json, docs/agents/codex.json",
                        });
            ...
            "tool.execute.before": async (input, sessionId) => {
                ...
                try {
                    ...
                    if (codexContexts.length === 0) {
                        await frameworkLogger.log('codex-injector', '-no-codex-loaded-allowing-action-but-enforcement-d', 'info', { message: "⚠️  No codex loaded - allowing action but enforcement disabled",
                        });
                    ...
                    if (!validation.compliant) {
                        ...
                        await frameworkLogger.log('codex-injector', '-recommendations-validation-recommendations-join-', 'info', { message: `💡 Recommendations: ${validation.recommendations.join(", ")}`,
                        });
            ...
            "tool.execute.after": async (input, output, sessionId) => {
                ...
                try {
                    ...
                    await frameworkLogger.log('codex-injector', '-stringray-tool-execution-hook-triggered-for-input', 'info', { message: `🔧 StringRay: Tool execution hook triggered for ${input.tool}`,
                    });
                    const codexContexts = await loadCodexContext(sessionId);
                    await frameworkLogger.log('codex-injector', '-stringray-loaded-codexcontexts-length-codex-conte', 'info', { message: `📚 StringRay: Loaded ${codexContexts.length} codex contexts`,
                    });
