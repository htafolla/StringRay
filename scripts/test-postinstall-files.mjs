...
class PostinstallFileValidator {
    ...
    async validateMCPConfig() {
        ...
        try {
            ...
            if (!fs.existsSync(mcpPath)) {
                ...
                this.results.failed.push({ test: 'MCP Config File', error: 'File does not exist' });
                return;
            }
            ...
            if (serverCount >= 20) {
                ...
                this.results.failed.push({ test: 'MCP Config File', error: `Only ${serverCount} servers` });
            }
            ...
            for (const [serverName, serverConfig] of Object.entries(mcpConfig.mcpServers || {})) {
                if (serverConfig.args && Array.isArray(serverConfig.args)) {
                    const hasConsumerPath = serverConfig.args.some(arg =>
            ...
            if (validPaths >= serverCount * 0.8) { // 80% have correct paths
                console.log(`  ✅ ${validPaths}/${serverCount} servers have correct consumer paths`);
                this.results.passed.push('MCP Server Paths');
            } else {
                console.log(`  ❌ Only ${validPaths}/${serverCount} servers have correct paths`);
                this.results.failed.push({ test: 'MCP Server Paths', error: 'Incorrect consumer paths' });
            }

        } catch (error) {
            console.log(`  ❌ MCP config validation failed: ${error.message}`);
            this.results.failed.push({ test: 'MCP Config File', error: error.message });
        }
    }
    ...
    async validateOpencodeConfig() {
        ...
        try {
            ...
            if (!fs.existsSync(opencodePath)) {
                ...
                this.results.failed.push({ test: 'OpenCode Config File', error: 'File does not exist' });
                return;
            }
            ...
            if (hasMCPConfig || hasAgentConfig) {
                ...
                console.log('  ⚠️ opencode.json exists but may not have StringRay config');
            ...
            this.results.failed.push({ test: 'OpenCode Config File', error: error.message });
        }
    }
    ...
    async validateOhMyOpencodeConfig() {
        ...
        try {
            ...
            if (!fs.existsSync(ohMyOpencodePath)) {
                ...
                this.results.failed.push({ test: 'oh-my-opencode Config', error: 'File does not exist' });
                return;
            }
            ...
            const hasPlugins = (ohMyOpencodeConfig.plugins && Array.isArray(ohMyOpencodeConfig.plugins)) ||
                              (ohMyOpencodeConfig.plugin && Array.isArray(ohMyOpencodeConfig.plugin));
            let hasStringRayPlugin = false;

            const pluginArray = ohMyOpencodeConfig.plugins || ohMyOpencodeConfig.plugin;
            ...
            if (hasStringRayPlugin) {
                ...
                this.results.failed.push({ test: 'oh-my-opencode Plugin Registration', error: 'Plugin not registered' });
            }
            ...
            console.log(`  ❌ oh-my-opencode config validation failed: ${error.message}`);
            this.results.failed.push({ test: 'oh-my-opencode Config', error: error.message });
        }
    }
    ...
    async validateOpencodeDirectories() {
        ...
        const directories = [
            { path: '.opencode', description: 'oh-my-opencode configuration directory' },
            { path: '.opencode/logs', description: 'Log directory (may not exist until runtime)' }
        ];
        ...
        for (const dir of directories) {
            try {
                ...
                if (dir.path.includes('logs') && !exists) {
                    ...
                    this.results.failed.push({ test: dir.description, error: 'Directory does not exist' });
                }
            } catch (error) {
                console.log(`  ❌ ${dir.description} check failed: ${error.message}`);
                this.results.failed.push({ test: dir.description, error: error.message });
            }
        }
    }
...
validator.validateFiles().then(success => {
