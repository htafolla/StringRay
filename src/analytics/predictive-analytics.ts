...
export class StrRayPredictiveAnalytics {
  ...
  predictOptimalAgent(
    ...
    const predictions = availableAgents.map(agent => {
      const confidence = this.calculateAgentConfidence(agent, taskType, complexity);
      const estimatedDuration = this.estimateTaskDuration(agent, taskType, complexity);
      const successProbability = this.calculateSuccessProbability(agent, taskType, complexity);
  ...
  private updateAgentMetrics(
    ...
    if (!existing) {
      ...
      const newScore = (currentScore * existing.totalTasks + (success ? 1 : 0)) / totalTasks;
      ...
      if (existing.recentPerformance.length > this.recentTasksWindow) {
        existing.recentPerformance = existing.recentPerformance.slice(-this.recentTasksWindow);
      }

      existing.averageExecutionTime = (existing.averageExecutionTime * existing.totalTasks + duration) / totalTasks;
      existing.averageComplexity = (existing.averageComplexity * existing.totalTasks + complexity) / totalTasks;
  ...
  private calculateAgentConfidence(
    ...
    const recentSuccessRate = agent.recentPerformance.reduce((a, b) => a + b, 0) / agent.recentPerformance.length;
    confidence += recentSuccessRate * 0.3;

    confidence += agent.successRate * 0.2;

    const complexityMatch = 1 - Math.abs(agent.averageComplexity - complexity) / Math.max(agent.averageComplexity, complexity);
  ...
  private estimateTaskDuration(
    ...
    const specializationBonus = (agent.specializationScore[taskType] || 0) * 0.2;
  ...
  private calculateSuccessProbability(
    ...
    const specializationBoost = (agent.specializationScore[taskType] || 0) * 0.2;
    probability += specializationBoost;

    const complexityPenalty = Math.abs(agent.averageComplexity - complexity) / 10;
  ...
  private analyzePatterns(): void {
    ...
    const taskTypeStats = new Map<string, { total: number; success: number; avgDuration: number }>();

    this.taskHistory.forEach(task => {
      const stats = taskTypeStats.get(task.taskType) || { total: 0, success: 0, avgDuration: 0 };
      stats.total++;
    ...
    taskTypeStats.forEach((stats, taskType) => {
      ...
      if (successRate > 0.9) {
        this.performancePatterns.push({
          ...
          recommendations: [`Prioritize ${taskType} tasks for high-reliability agents`]
        });
      }
      ...
      if (stats.avgDuration > 5000) {
        this.performancePatterns.push({
          ...
          impact: 'negative',
          confidence: 0.8,
          recommendations: [`Consider optimizing ${taskType} task processing`, `Evaluate agent assignment for ${taskType} tasks`]
        });
      }
    });
    ...
    recentTasks.forEach(task => {
      agentWorkload.set(task.agentId, (agentWorkload.get(task.agentId) || 0) + 1);
    });
    ...
    agentWorkload.forEach((workload, agentId) => {
      ...
      if (percentage > 0.4) {
        this.performancePatterns.push({
          ...
          impact: 'negative',
          confidence: 0.9,
          recommendations: [`Redistribute tasks from overloaded agent ${agentId}`, `Consider adding more agents for load balancing`]
        });
      }
      ...
      if (percentage < 0.05) {
        this.performancePatterns.push({
          ...
          impact: 'neutral',
          confidence: 0.8,
          recommendations: [`Consider assigning more tasks to agent ${agentId}`, `Evaluate if agent specialization matches current workload`]
        });
      }
    });
  }

  private generateRecommendations(agentMetrics: AgentPerformanceMetrics[]): string[] {
    ...
    if (topPerformers.length > 0) {
      recommendations.push(`Top performing agents: ${topPerformers.map(a => a.agentId).join(', ')}`);
    }

    if (underPerformers.length > 0) {
      recommendations.push(`Consider reviewing performance of: ${underPerformers.map(a => a.agentId).join(', ')}`);
    }
    ...
    if (specializationOpportunities.length > 0) {
      recommendations.push('Leverage agent specializations: ' +
        specializationOpportunities.map(s => `${s.agentId} excels at ${s.topSpecialty}`).join('; '));
    }

    return recommendations;
  }

  private calculateOverallEfficiency(agentMetrics: AgentPerformanceMetrics[]): number {
    if (agentMetrics.length === 0) return 0;

    const totalTasks = agentMetrics.reduce((sum, a) => sum + a.totalTasks, 0);
    const weightedSuccessRate = agentMetrics.reduce((sum, a) => sum + (a.successRate * a.totalTasks), 0) / totalTasks;

    const specializationDiversity = agentMetrics.reduce((sum, a) => sum + Object.keys(a.specializationScore).length, 0) / agentMetrics.length;

    return (weightedSuccessRate * 0.7) + (specializationDiversity / 10 * 0.3);
  }

  private identifyBottlenecks(agentMetrics: AgentPerformanceMetrics[]): string[] {
    return agentMetrics
  ...
  private identifyUnderutilizedAgents(agentMetrics: AgentPerformanceMetrics[]): string[] {
    const totalTasks = agentMetrics.reduce((sum, a) => sum + a.totalTasks, 0);
