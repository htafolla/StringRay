...
class StrRayCodeReviewServer {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                focusAreas: {
                  ...
                  items: {
                    type: "string",
                    enum: ["security", "performance", "maintainability", "style", "logic"]
                  },
  ...
  private async analyzeCodeQuality(args: any): Promise<any> {
    ...
    try {
      ...
      const metrics = includeMetrics ? this.calculateMetrics(content, language) : null;
  ...
  private analyzeCode(content: string, language: string, focusAreas?: string[]): CodeIssue[] {
  ...
  private analyzeSecurity(content: string, lines: string[], language: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
  ...
  private analyzePerformance(content: string, lines: string[], language: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      // Inefficient loops
      if (line.includes('for') && line.includes('.length') && !line.includes('let ')) {
        issues.push({
          type: 'warning',
  ...
  private analyzeMaintainability(content: string, lines: string[], language: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
      ...
      functions.forEach(func => {
        ...
        if (funcLines > 50) {
          issues.push({
            ...
            line: lines.findIndex(l => l.includes(func.split(' ')[1] || func)) + 1,
  ...
  private analyzeStyle(content: string, lines: string[], language: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
    ...
    lines.forEach((line, index) => {
      ...
      // Magic numbers
      if (/\b\d{2,}\b/.test(line) && !line.includes('//') && !line.includes('/*')) {
  ...
  private analyzeLogic(content: string, lines: string[], language: string): CodeIssue[] {
    const issues: CodeIssue[] = [];
  ...
  private generateSuggestions(issues: CodeIssue[], metrics: CodeMetrics | null, language: string): string[] {
  ...
  private calculateOverallScore(issues: CodeIssue[], metrics: CodeMetrics | null): number {
    let score = 100;
  ...
  private assessOverallQuality(avgScore: number, totalIssues: number, criticalIssues: number): string {
  ...
  private checkStandardsCompliance(content: string, language: string, standards: string[]): any[] {
  ...
  private generateStandardsRecommendations(violations: any[], language: string, standards: string[]): string[] {
