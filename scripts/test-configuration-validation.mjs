  ...
  async validateMCPConfigPaths() {
    ...
    try {
            ...
            if (typeof arg === "string") {
              ...
              // Check if path points to existing file
                ...
                if (fs.existsSync(fullPath)) {
                  ...
                  console.log(
                    `  ⚠️ MCP server ${serverName} path does not exist: ${arg}`,
                  );
                }
              }
            }
      ...
      if (validPaths === totalPaths && totalPaths > 0) {
        console.log(
          `  ✅ All ${totalPaths} MCP server paths are valid consumer paths`,
        );
        this.results.passed.push("MCP Config Paths");
      } else {
        console.log(
          `  ❌ ${validPaths}/${totalPaths} MCP server paths are valid`,
        );
        this.results.failed.push({
          test: "MCP Config Paths",
          error: `${totalPaths - validPaths} invalid paths`,
        });
      }
    } catch (error) {
  ...
  async validateOpencodeConfigPaths() {
    ...
    try {
      ...
      if (issues.length === 0) {
        ...
        this.results.failed.push({
          test: "OpenCode Config Paths",
          error: `${issues.length} paths need updating`,
        });
      }
    } catch (error) {
      console.log(
        `  ❌ OpenCode config path validation failed: ${error.message}`,
      );
      this.results.failed.push({
        test: "OpenCode Config Paths",
        error: error.message,
  ...
  async validateOhMyOpencodePluginPaths() {
    ...
    try {
      ...
      if (!fs.existsSync(ohMyOpencodePath)) {
        ...
        this.results.failed.push({
          test: "oh-my-opencode Plugin Paths",
          error: "File does not exist",
        });
        return;
      }
      ...
      if (
        ...
        for (const plugin of ohMyOpencodeConfig.plugins) {
          if (typeof plugin === "string") {
            totalPlugins++;
            if (
              plugin.includes("../node_modules/strray-ai/") ||
            ...
            totalPlugins++;
            if (
              plugin.path.includes("../node_modules/strray-ai/") ||
      ...
      if (validPlugins === totalPlugins && totalPlugins > 0) {
        ...
        console.log(
          `  ❌ ${validPlugins}/${totalPlugins} plugin paths are valid`,
        );
        this.results.failed.push({
          test: "oh-my-opencode Plugin Paths",
          error: `${totalPlugins - validPlugins} invalid paths`,
        });
      }
    } catch (error) {
      console.log(
        `  ❌ oh-my-opencode plugin path validation failed: ${error.message}`,
      );
      this.results.failed.push({
        test: "oh-my-opencode Plugin Paths",
        error: error.message,
    ...
    for (const config of configs) {
      try {
        ...
        if (!fs.existsSync(configPath)) {
          ...
          this.results.failed.push({
            test: `${config.description} Integrity`,
            error: "File missing",
          });
          continue;
        }
        ...
        console.log(
          `  ❌ ${config.description} validation failed: ${error.message}`,
        );
        this.results.failed.push({
          test: `${config.description} Integrity`,
          error: error.message,
